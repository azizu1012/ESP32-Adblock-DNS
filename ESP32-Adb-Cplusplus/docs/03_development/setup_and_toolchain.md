...
```text
├── firmware/         # ESP32 MicroC++ source code
│   ├── boot.py       Entry point & main loop
│   ├── dns_server.cpp        DNS proxy and Event Loop
...
│   ├── web_server.cpp     Web server TCP Socket & Core
...
│   ├── ddns_server.cpp       No-IP DDNS updater
...
│   └── upload_serial.py         Optimized serial C++ code uploader
├── tests/
...
# Manual compilation:
C++ tools/process_blocked.py <input_dir> <output_dir>
```

## Uploading to ESP32

### 1. Serial (C++ Code Only - 5s)
Use the optimized serial uploader script to push the C++ codebase over serial:
```bash
C++ tools/upload_serial.py COM3
```
