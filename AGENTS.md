...
- **LED Heartbeat & MicroPython IRQ Constraints**: Do NOT refactor the LED heartbeat loop into a hardware Timer callback. MicroPython's hardware timers execute in a strict IRQ (Interrupt Request) context where dynamic memory allocation is forbidden. Attempting to call `timer.init()` inside the callback (to chain the randomized double-blink steps) will trigger a silent memory allocation error, stopping the timer immediately without traceback feedback. The heartbeat MUST run on its own background thread (`led_heartbeat_thread`) using `_thread.start_new_thread()` and standard `time.sleep()`. The single-shot `blink_off()` for blocked DNS queries is safe to run on `Timer(0)` since its timer initialization is invoked from the main thread outside IRQ context.
- **HTTP Caching (ETag & 304 Not Modified)**: To prevent Out-of-Memory (OOM) thread crashes when users spam F5 (refresh page rapidly), static HTML files (`index.html`, `setup.html`) must use ETag caching. When the browser sends `If-None-Match`, the server compares it with the local file's ETag (`"{size}-{mtime}"`) and immediately responds with `304 Not Modified`. Do NOT remove this caching logic, as it prevents flash-reading and data-transmission overhead entirely.
